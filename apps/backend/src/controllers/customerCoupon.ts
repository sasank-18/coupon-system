...
import { redeemCouponSchema } from "@repo/common/types";
...
export const redeemCoupon = async (req: Request, res: Response) => {
  try {  
    ...
    if (!orderAmount) {
      return res.status(400).json({
        success: false,
        message: "orderAmount is required to calculate discount",
      });
    }
    ...
    if (!coupon) {
      return res.status(404).json({
        success: false,
        message: "Invalid or non-existing coupon code",
      });
    }
    ...
    if (coupon.redemptions_used >= coupon.max_redemptions) {
      return res.status(400).json({
        success: false,
        message: "Coupon has reached its maximum number of usages",
      });
    }
    ...
    if (coupon.discount_type === "FIXED") {
      ...
      discountAmount = coupon.max_discount
        ? Math.min(calculated, coupon.max_discount)
        : calculated;
    }
